...
void show_hdd(void) {
  ...
  scr_printf("\n  HDD: %lu MB free of %lu MB (max-part %lu MB)\n",
             (unsigned long)info.hddFree, (unsigned long)info.hddSize,
...
int execute_install(const install_plan_t *plan) {
  ...
  snprintf(create_cmd, sizeof(create_cmd), "%s,,,%s,HDL", hdd_path,
           plan->main_size_str);
  ...
  for (i = 0; i < plan->subs_needed; i++) {
    ...
    int sret =
        fileXioIoctl2(fd, HIOCADDSUB, (char *)sz, strlen(sz) + 1, NULL, 0);
    if (sret < 0) {
  ...
  strncpy(args.GameTitle, plan->volume_id, sizeof(args.GameTitle) - 1);
  strncpy(args.StartupPath, plan->startup_id, sizeof(args.StartupPath) - 1);

  scr_printf("  fileXioFormat: %s\n", hdd_path);
  int ret = fileXioFormat("hdl0:", hdd_path, (const char *)&args, sizeof(args));
  if (ret < 0) {
...
int stream_iso_to_partition(const install_plan_t *plan, const char *iso_path) {
  char hdd_path[64];
  snprintf(hdd_path, sizeof(hdd_path), "hdd0:%s", plan->partition_name);
  ...
  while (written < total) {
    ...
    if (pct != last_pct || now != last_render) {
      ...
      if (elapsed > 0 && written > 0) {
        ...
        eta = (unsigned)((remaining * (uint64_t)elapsed) / written);
        mbps10 = (unsigned)((written * 10ULL) >> 20) / (unsigned)elapsed;
      }
      ...
      scr_printf("  %u/%u MB  %u.%u MB/s  %u:%02u elapsed  ETA %u:%02u",
                 (unsigned)(written >> 20), total_mb, mbps10 / 10, mbps10 % 10,
                 (unsigned)(elapsed / 60), (unsigned)(elapsed % 60), eta / 60,
                 eta % 60);
      last_pct = pct;
      last_render = now;
    }
